...
def intake(request: IntakeRequest) -> dict[str, Any]:
    """Create a staff-facing patient intake record for the Streamlit app."""

    with triage_lock:
        return triage_service.intake(request)


@app.get("/queue")
def queue() -> dict[str, object]:
    """Return the live patient queue in the exact shape expected by the frontend."""

    with triage_lock:
        return {"patients": triage_service.queue()}


@app.get("/alerts")
def alerts() -> dict[str, object]:
    """Return emergency escalation alerts for the dashboard."""

    with triage_lock:
        return triage_service.alerts()


@app.get("/triage-logic/{patient_id}")
def triage_logic(patient_id: str) -> dict[str, Any]:
    """Return the priority-score explanation for a patient."""

    with triage_lock:
        logic = triage_service.logic_for(patient_id)
    if logic is None:
        raise HTTPException(status_code=404, detail={"message": f"Unknown patient_id '{patient_id}'."})
    return logic


@app.get("/system-insights")
def system_insights() -> dict[str, Any]:
    """Return compact system-level metrics for the dashboard header."""

    with triage_lock:
        return triage_service.system_insights()


@app.post("/demo/seed")
def seed_demo() -> dict[str, Any]:
    """Load demo patients for quick judging/demo walkthroughs."""

    with triage_lock:
        return triage_service.seed_demo_patients()
...
def main() -> None:
    uvicorn.run("backend.server.app:app", host="0.0.0.0", port=7860)
